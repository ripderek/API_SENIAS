...
const login_register_participante = async (req, res, next) => {
    try {
        ...
        if (hd === "uteq.edu.ec") {
            ...
            const users = await pool.query('select * from auth_data_participantes($1)', [email]);
            ...
            if (!result) {
                ...
                const result = await pool.query('call sp_registrar_participantes($1,$2)', [email, name]);
            }
            ...
            const data_auth = await pool.query('select * from id_participante_emil($1)', [email]);
            let data = data_auth.rows[0];
            ...
            return res.status(404).json({ error: "El correo no pertenece a la UTEQ" });
...
const verificaUserGoogle = async (req, res, next) => {
    try {
        ...
        //Ver si el usuario es admin de area y guardar en json para que se guarde como cookie
        return res.status(200).json({ verification: "true", token: token, id: userd });
